...
public class SkillNameAlreadyExistsImpl implements ConstraintValidator<SkillNameAlreadyExists, String> {
    ...
    public boolean isValid(String skillName, ConstraintValidatorContext constraintValidatorContext) {
        return (httpServletRequest.getMethod().equals(HttpMethod.POST.name()))?(!skillRepository.existsByName(skillName)):validateSkillNameAlreadyExistsWithSameID(skillName); //if name exists(true), method should not be valid
    }
